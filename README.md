...
The staging site is live at https://dgilsep5ai167.cloudfront.net/. Commit and push from VS Code normally to update GitHub and run validation. To publish a revision to AWS, open GitHub **Actions → Deploy or roll back staging → Run workflow**, select `main`, and leave the release field blank. Deployment is manual; an ordinary push does not change the live site. Enter a retained release ID only when rolling back.
